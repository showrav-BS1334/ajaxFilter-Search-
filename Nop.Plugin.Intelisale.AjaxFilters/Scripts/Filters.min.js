        ...
        x.waitForAjaxRequest ||
          ("#/noFilterApplied" !== n &&
            (e && t && k() && m.address.history(!0),
              ...
              $('.custom-loading').show(),
              $(".scroll-margin")[0].scrollIntoView({
              ...
              m
                    ...
                    F.refreshFilters(e),
                      ...
                      (F.expandFilterPanelsWithSelectedOptions(),
                        "" !== (t = m(e).filter("#urlHashQuery").val().toString())
                          ...
                          : c || m(this).attr("data-pageNumber") == undefined ? m.address.value("") : m.address.value("noFilterApplied")),
                      F.hideProductPanelAjaxBusy(), $('.custom-loading').hide(),
                ...
                .always(function () {
                  ...
                  // intelisale custom code
                  addOrRemoveMetaTagRobot(window.location.href);
                })));
      }),
      ...
      (x.prototype.addProductPanelAjaxBusyToPage = function () {
        ...
        if (window.innerWidth > 767) {
          if (e.hasClass("product-list")) {
            $(".product-list-view-header").css('display', 'block');
          }
          else if (e.hasClass("product-grid")) {
            $(".product-list-view-header").css('display', 'none');
          }
        }
      }),
...
// intelisale custom code
function addOrRemoveMetaTagRobot(url) {
  var hasQueryParam = false;

  // check url with '#/'
  var urlWithQueryParams = url.split('#/');
  if (urlWithQueryParams != null &&
    urlWithQueryParams.length > 1 &&
    urlWithQueryParams[1].indexOf("=") !== -1) {
    hasQueryParam = true;
  }
  ...
  if (hasQueryParam) {
    // Remove meta tag robots
    if (metaTag != undefined && metaTag != null) {
      var parentElement = metaTag.parentNode;
      parentElement.removeChild(metaTag);
    }
  }
  else {
    ...
    if (metaTag == undefined || metaTag == null) {
      ...
      newMetaTag.setAttribute('content', 'index,follow');
